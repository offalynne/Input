...
function input_binding_set_safe(_verb, _binding, _player_index = 0, _alternate = 0)
{
    if (input_value_is_binding(_binding))
    {
        ...
        else
        {
            ...
            var _config_name = global.__input_players[_player_index].get_binding_config_name(_binding);
            ...
            if ((_verb != _verb_b) || (_alternate != _alternate_b))
            {
                ...
                input_binding_swap(_verb, _alternate, _verb_b, _alternate_b, _config_name, _player_index);
            }
            else
            {
                __input_trace("New binding (", input_binding_get_name(_binding), ") is the same as existing binding for config=", _config_name, ", verb=", _verb, ", alternate=", _alternate);
            }
        }
