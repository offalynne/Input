...
function input_mouse_capture_set(_state, _sensitivity = 1)
{
    ...
    {
        if (__INPUT_DEBUG_CAPTURE) __input_trace(true, "Mouse capture unsupported for this platform");
        return;
    }
    ...
    switch(os_type)
    {
        case os_windows: //Window area intersecting the border on any monitor invalidates mouse capture position
            ...
            repeat((array_length(_monitor_coords) div 8))
            {
                ...
                {
                    if (__INPUT_DEBUG_CAPTURE) __input_trace(true, "Window exceeds display bounds on monitor ", _i + 1, ", mouse capture blocked");
        ...
        case os_macosx: //Window area bordering or outside the primary monitor invalidates mouse capture position
            ...
            {
                if (__INPUT_DEBUG_CAPTURE) __input_trace(true, "Window exceeds display bounds on primary monitor, mouse capture blocked");
                _global.__mouse_capture_blocked = true;
                return;
        ...
        case os_linux: //No window_get_visible_rect() values but we can test top left and primary monitor intersection
            ...
            {
                if (__INPUT_DEBUG_CAPTURE) __input_trace(true, "Window exceeds display bounds, mouse capture blocked");
                _global.__mouse_capture_blocked = true;
                return;
            }            
        break;
