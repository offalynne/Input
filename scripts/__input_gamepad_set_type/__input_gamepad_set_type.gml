            ...
            if (os_type == os_linux)
            {
                ...
                switch (_wii_type_match)
                {
                    ...
                    case "WiiClassic":
                        ...
                        repeat(index)
                        {
                            ...
                            if (input_gamepad_is_connected(_g))
                            {
                                ...
                                if ((INPUT_GAMEPAD[@ _g].raw_type == "WiiRemote")
                                ||  (INPUT_GAMEPAD[@ _g].raw_type == "WiiMotionPlus"))
                                {
                    ...
                    case "WiiRemote":                    
                        ...
                        if ((_g >= 1)
                        &&  (INPUT_GAMEPAD[@ _g].hat_count    == 4)
                        &&  (INPUT_GAMEPAD[@ _g].button_count == 0)
                        &&  (INPUT_GAMEPAD[@ _g].axis_count   == 0))
                        {
                            //Found IR sensor
                            var _ir_index = _g;
                            repeat(_ir_index + 1)
                            ...
                            if ((_g >= 0)
                            &&  (INPUT_GAMEPAD[@ _g].axis_count   == 3)
                            &&  (INPUT_GAMEPAD[@ _g].button_count == 0)
                            &&  (INPUT_GAMEPAD[@ _g].hat_count    == 0))
                            {
                                //Found IMU                                
                                var _imu_index = _g;
                                __input_trace("Overriding controller ", _imu_index ," type to \"WiiRemoteIMU\"");
                                with (INPUT_GAMEPAD[@ _imu_index])
                                {
