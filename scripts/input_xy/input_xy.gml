...
function input_xy(_verb_l, _verb_r, _verb_u, _verb_d, _player_index = 0, _most_recent = INPUT_DEFAULT_2D_MOST_RECENT)
{
    ...
    if (((_value_l > 0.0) && !_verb_struct_l.__rawAnalogue)
    ||  ((_value_r > 0.0) && !_verb_struct_r.__rawAnalogue)
    ||  ((_value_u > 0.0) && !_verb_struct_u.__rawAnalogue)
    ||  ((_value_d > 0.0) && !_verb_struct_d.__rawAnalogue))
    {
        ...
        if (_verb_struct_u.__rawAnalogue) _value_u = 0.0;
        if (_verb_struct_d.__rawAnalogue) _value_d = 0.0;
        if (_verb_struct_l.__rawAnalogue) _value_l = 0.0;
        if (_verb_struct_r.__rawAnalogue) _value_r = 0.0;
    }
    ...
    if (_value_l > 0.0) { _active_count++;   _min_threshold += _verb_struct_l.__minThreshold;   _max_threshold += _verb_struct_l.__maxThreshold; }
    if (_value_r > 0.0) { _active_count++;   _min_threshold += _verb_struct_r.__minThreshold;   _max_threshold += _verb_struct_r.__maxThreshold; }
    if (_value_u > 0.0) { _active_count++;   _min_threshold += _verb_struct_u.__minThreshold;   _max_threshold += _verb_struct_u.__maxThreshold; }
    if (_value_d > 0.0) { _active_count++;   _min_threshold += _verb_struct_d.__minThreshold;   _max_threshold += _verb_struct_d.__maxThreshold; }
