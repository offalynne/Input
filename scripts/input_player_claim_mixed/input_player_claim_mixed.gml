...
function input_player_claim_mixed(_allow_keyboard, _allow_mouse, _allow_gamepads, _player_index = 0)
{
    ...
    if (_allow_keyboard && !global.__input_keyboard_default_defined)
    {
    ...
    if (_allow_mouse && !global.__input_mouse_default_defined)
    {
        __input_trace("Warning! Player ", _player_index, " cannot claim the mouse, no mouse bindings have been created");
        _allow_mouse = false;
    }
    
    if (_allow_gamepads && !global.__input_keyboard_default_defined)
    {
    ...
    if (_allow_keyboard || (_allow_mouse && INPUT_KEYBOARD_AND_MOUSE_ALWAYS_PAIRED)) global.__input_players[_player_index].__add_source(INPUT_SOURCE.KEYBOARD);
    if (_allow_mouse || (_allow_keyboard && INPUT_KEYBOARD_AND_MOUSE_ALWAYS_PAIRED)) global.__input_players[_player_index].__add_source(INPUT_SOURCE.MOUSE);
    if (_allow_gamepads) global.__input_players[_player_index].__add_source(INPUT_SOURCE.ALL_GAMEPADS);
}
