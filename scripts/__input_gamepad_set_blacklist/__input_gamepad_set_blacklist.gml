...
function __input_gamepad_set_blacklist()
{
    ...
    if ((axis_count == 0) && (button_count == 0) && (hat_count == 0))
    {
        ...
        __input_trace(true, "Warning! Controller ", index, " (VID+PID \"", vendor + product, "\") blacklisted: no button or axis");
        blacklisted = true;
        return;
    }
    ...
    if (__INPUT_ON_WINDOWS)
    {
        if ((vendor == "7e05") && (product == "0920") && (button_count > 21))
        {
           ...
           __input_trace(false, "Warning! Controller is blacklisted (Switch Pro Controller over USB)");
            blacklisted = true;
            return;
        }
         ...
         || ((axis_count ==  8) && (button_count == 0))))  //DsHidMini gyro
        {
           ...
           __input_trace(false, "Warning! Controller is blacklisted (Incorrectly configured PS3 controller)");
            blacklisted = true;
            return;
        }
    }
    else if (__INPUT_ON_LINUX)
    {
        if (_global.__on_steam_deck)
        {
            if ((button_count == 144) && (axis_count == 0))
            {
               ...
               __input_trace(false, "Warning! Controller ", index, " is blacklisted (Steam Deck virtual keyboard)");
                blacklisted = true;
                return;
            }
        ...
        if ((button_count == 0) && (axis_count == _joycon_imu_axis_count) && (hat_count == 0))
        {
            ...
            if (_i == _joycon_imu_axis_count)
            {
               ...
               __input_trace(false, "Warning! Controller ", index, " blacklisted (matches Joy-Con motion unit)");
                blacklisted = true;
                return;
            }
        }
        ...
        ||  (raw_type == "HIDWiiRemoteIMU")  || (raw_type == "HIDWiiRemoteIRSensor"))
        {
           ...
           __input_trace(false, "Warning! Controller ", index, " is blacklisted, type (\"", raw_type, "\")");
            blacklisted = true;
            return;
        }
    }
    ...
    if ((vendor != "de28") && variable_struct_exists(_global.__ignore_gamepad_types, simple_type))
    {
       ...
       __input_trace(false, "Warning! Controller type is blacklisted by Steam Input (\"", simple_type, "\")");
        blacklisted = true;
        return;
    }
    ...
    {
       __input_trace(false, "Warning! Controller is blacklisted (found by GUID \"", guid, "\")");
        blacklisted = true;
        return;
    }
    else if (is_struct(_os_vidpid_dict) && variable_struct_exists(_os_vidpid_dict, string(vendor) + string(product)))
    {
       __input_trace(false, "Warning! Controller is blacklisted (found by VID+PID \"", vendor, product, "\")");
        blacklisted = true;
        return;
    }
    else if (is_array(_os_desc_array))
    {
        ...
        repeat(array_length(_os_desc_array))
        {
            ...
            {
               __input_trace(false, "Warning! Controller is blacklisted (banned substring \"", _os_desc_array[_i], "\" found in description)");
                blacklisted = true;
                return;
            }
