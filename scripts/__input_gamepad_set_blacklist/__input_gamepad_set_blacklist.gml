...
function __input_gamepad_set_blacklist()
{
    ...
    if ((__axis_count == 0) && (__button_count == 0) && (__hat_count == 0))
    {
        ...
        __input_trace("Warning! Controller ", __index, " (VID+PID \"", __vendor + __product, "\") blacklisted: no button or axis");
        __blacklisted = true;
        return;
    }
        ...
        case os_linux:
            ...
            if ((__button_count == 0) && (__axis_count == _joycon_imu_axis_count) && (__hat_count == 0))
            {
                ...
                if (_i == _joycon_imu_axis_count)
                {
                    ...
                    if (!__INPUT_SILENT) __input_trace("Warning! Controller ", __index, " blacklisted (matches Joy-Con motion unit)");
                    __blacklisted = true;
                    return;
                }
            }
