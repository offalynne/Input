...
function __input_gamepad_guid_parse(_guid, _legacy, _suppress)
{
    ...
    {
        if (!_suppress) __input_trace(false, "Warning! GUID was empty");
    ...
    else
    {
        ...
        else
        {
            ...
            {
                if (!_suppress) __input_trace(false, "Warning! GUID \"", _guid, "\" does not fit expected pattern. VID+PID cannot be extracted");
                return { vendor : "", product : "", description : "" };
            }
            ...
            {
                if (!_suppress) __input_trace(false, "Warning! GUID \"", _guid, "\" driver ID does not match expected (Found ", string_copy(_guid, 1, 4), ", expect either 0300 or 0500)");
            }
