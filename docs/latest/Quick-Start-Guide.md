...
Just like the keyboard functions, gamepad functions use [normal gamepad input values](https://manual.yoyogames.com/GameMaker_Language/GML_Reference/Game_Input/GamePad_Input/Gamepad_Input.htm) from standard GM features. Congratulations, you now have gamepad input!  In order for the player to be able to switch between keyboard and gamepad devices on the fly, add <code>[input_source_hotswap_tick()](input_source_hotswap_tick);</code> to your Step event.
