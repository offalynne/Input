...
Issues a modal prompt for text-entry. Intended for use on console platform targets. Requires use of `input_string_dialog_async_event`. Note that this function is blocked on Linux, iOS and tvOS target platforms.
...
Sets internal string to the result of a text-entry prompt. Must be used in the context of an [_Asynchronous Dialogue event_](https://manual.yoyogames.com/The_Asset_Editors/Object_Properties/Async_Events/Dialog.htm). Required for use of `input_string_async_get`.
