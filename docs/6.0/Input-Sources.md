...
A mobile touchscreen, a Switch touchscreen, a Steam Deck touchscreen (Windows only), or a PlayStation touchpad. If `INPUT_TOUCHSCREEN_USES_MOUSE_SOURCE` is set to `true` then all `INPUT_TOUCH` devices will be treated as `INPUT_MOUSE` instead.
